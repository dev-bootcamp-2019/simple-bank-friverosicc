...
contract SimpleBank {
    ...
    function withdraw(uint withdrawAmount) public payable returns (uint) {
      ...
      if (enrolled[client]) {
        if (balances[client] <= withdrawAmount) {
          (bool transactionIsOk, bytes memory data) = client.call.value(withdrawAmount)("");

          if (transactionIsOk) {
            uint currentBalance = balances[client];
            balances[client] = currentBalance - withdrawAmount;

            emit LogWithdrawal(client, withdrawAmount, balances[client]);
